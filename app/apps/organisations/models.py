...
class Organisation(models.Model):
    ...
    org_working_hours = models.JSONField(
        blank=True, null=True, default=default_working_hours
    )
    ...
    org_admins = models.ManyToManyField(User, blank=True, related_name="org_admins")
    ...
    def save(self, *args, **kwargs):
        if not self.org_slug:
            max_org_id = (
                Organisation.objects.aggregate(models.Max("org_id"))["org_id__max"] or 0
            )
            org_id = max_org_id + 1
            ...
            try:
                old_org_name = (
                    Organisation.objects.filter(org_id=self.org_id)
